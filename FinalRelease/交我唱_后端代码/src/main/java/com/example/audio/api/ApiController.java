...
//			jdbcTemplate.execute("insert into search values("+song_id+",\""+song_name+"\",\""+song_picture_url+"\" , \""+artist_name+"\");");
	...
	public @ResponseBody JSONObject songGet(@PathVariable("int") int song_id) throws Exception {
		api.info("### /songs "+song_id+"\n");
		JSONObject jsonObject = new JSONObject();
		String mp3_path = getUrl(song_id);
		String lyric = getLyric(song_id);
		String artist = getArtist(song_id);
		String song_name = getSongname(song_id);
		String album_name = getAlbumname(song_id);
		String album_pic = getAlbumpic(song_id);
...
//		String local_path="/home/waaa/jwc/audioData/music/"+filename+".mp3";
		String local_path="D:/Data/kAPP/audioData/music/"+filename+".mp3";
		writeToLocal(local_path, in);

		if(jdbcTemplate.queryForObject("select count(*) from search where id = \""+song_id+"\";", int.class)==0){
			jdbcTemplate.execute("insert into search values("+song_id+",\""+song_name+"\",\""+album_pic+"\" , \""+artist+"\" , \""+album_name+"\");");
		}
		...
		jsonObject.put("songname",song_name);
		jsonObject.put("albumname",album_name);
		jsonObject.put("albumpic",album_pic);
...
//		System.out.println(end);
		return doScore("http://45.253.64.55:11455/score", formList);
	}
...
//		score++;
	...
	public String getArtist(int song_id) throws Exception {
		String url = "http://api.imjad.cn/cloudmusic/?type=detail&id=" + song_id;
		JSONObject response_json = doGet(url);
		return response_json.getJSONArray("songs").getJSONObject(0).getJSONArray("ar").getJSONObject(0).getString("name");
	}

	public String getSongname(int song_id) throws Exception {
		String url = "http://api.imjad.cn/cloudmusic/?type=detail&id=" + song_id;
		JSONObject response_json = doGet(url);
		return response_json.getJSONArray("songs").getJSONObject(0).getString("name");
	}

	public String getAlbumname(int song_id) throws Exception {
		String url = "http://api.imjad.cn/cloudmusic/?type=detail&id=" + song_id;
		JSONObject response_json = doGet(url);
		return response_json.getJSONArray("songs").getJSONObject(0).getJSONObject("al").getString("name");
	}

	public String getAlbumpic(int song_id) throws Exception {
		String url = "http://api.imjad.cn/cloudmusic/?type=detail&id=" + song_id;
		JSONObject response_json = doGet(url);
		return response_json.getJSONArray("songs").getJSONObject(0).getJSONObject("al").getString("picUrl");
	}
